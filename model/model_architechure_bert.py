#model_architechure_bert_multi_scale_multi_loss
import os
import torch
...
class DocumentBertScoringModel():
    def __init__(self, args=None):
        ...
        self.bert_regression_by_word_document = DocumentBertCombineWordDocumentLinear.from_pretrained(
            ...
            "/content/drive/MyDrive/hw_ner/ASAP/Model/normalize_model_8_linear.bin",
            config=config
        )
        self.bert_regression_by_chunk = DocumentBertSentenceChunkAttentionLSTM.from_pretrained(
            # self.args['bert_model_path'] + "/chunk",
            "/content/drive/MyDrive/hw_ner/ASAP/Model/normalize_model_8_chunk.bin",
            config=config)

    def predict_for_regress(self, data):
        correct_output = None
        if isinstance(data, tuple) and len(data) == 2: #輸入是(test_documents, test_labels)，
        ...
        with torch.no_grad():
            ...
            for i in range(0, document_representations_word_document.shape[0], self.args['batch_size']):
                ...
                batch_predictions_chunked = torch.zeros_like(batch_predictions_word_document)
                for chunk_index in range(len(self.chunk_sizes)):
                    ...
                    batch_predictions_chunked += batch_predictions_chunk
                batch_predictions_word_chunk_sentence_doc = torch.add(batch_predictions_word_document, batch_predictions_chunked)
                predictions[i:i + self.args['batch_size']] = batch_predictions_word_chunk_sentence_doc.squeeze()
        ...
        inverse_predictions = scaler.inverse_transform(predictions_numpy).flatten()
        inverse_predictions = np.around(inverse_predictions)

        print(inverse_predictions)

        # prediction_scores.append(fix_score(item, self.prompt))
        test_eva_res = evaluation(correct_output, inverse_predictions)
