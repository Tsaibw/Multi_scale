...
class DocumentBertSentenceChunkAttentionLSTM(BertPreTrainedModel):
    def __init__(self):
        ...
        self.linear = nn.Linear(bert_model_config.hidden_size, 100)
...
class DocumentBertCombineWordDocumentLinear(BertPreTrainedModel):
    def __init__(self):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(bert_model_config.hidden_size * 2, 100)
        )
...
class MultiTrait(nn.Module):
    def __init__(self, args):
        ...
        self.cross_atten = nn.MultiheadAttention(100, args.mhd_head, batch_first=True)
        self.linear = SoftAttention(args.hidden_dim)
    ...
    def forward(self, doc_fea, chunk_fea, readability, hand_craft):
        ...
        cross_fea = self.linear(cross_out)
        trait_fea = torch.cat([cross_fea, readability, hand_craft], dim=-1)
...
class Scorer(nn.Module):
    def __init__(self, args):
        ...
        self.trait_att = nn.MultiheadAttention(186, 1, batch_first=True)
        self.score_layer = nn.Linear(186, 1, bias=True)
...
class multiBert(nn.Module):
    ...
    def _get_mask(self, target):
        mask = torch.ones(*target.size(), device=target.device, requires_grad=True)
        mask.data.masked_fill_((target == -1), 0)
        return mask.to(torch.bool)


    def evaluate(self, eval_loader, test_loader, epoch, evaluator, device="cuda"):
        self.eval()
        with torch.no_grad():
            
            eval_loss, eval_preds = self.compute_predictions(eval_loader, device)
            test_loss, test_preds = self.compute_predictions(test_loader, device)
            results = evaluator.evaluate(eval_preds, test_preds, epoch)
            return eval_loss / len(eval_loader), test_loss/len(test_loader), results

    
    def compute_predictions(self, dev_loader, device):
        dev_loss = 0
        dev_preds = []
        ...
        for batch in tqdm(dev_loader):
            ...
            loss, predict_score, scaled_score = self.forward(
                    ...
                    hand_craft = hand_craft.to(device),
                    scaled_score = scaled_score
            )

            
            dev_loss += loss.item()
            eval_pred = predict_score.detach().cpu().numpy()     
            for i in eval_pred:
                dev_preds.append(i)
        
        return dev_loss, dev_preds
